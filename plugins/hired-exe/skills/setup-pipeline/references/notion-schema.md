...
| Application Status | Status | To Review, Submitted, Phone Screen, Interviewing, Offer, Rejected, Withdrawn, Passed |
| Fit Score | Select | Very High, High, Medium, Low, Very Low |
| Posting | URL | The posting link. Store the raw URL, dedup on the normalized form |
...
2. **Needs action** - table, filtered to Fit Score is High or Very High and Application
   Status is To Review, sorted by Date Added descending. This is the view the user should
   actually check each morning.
